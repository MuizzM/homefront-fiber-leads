...
export function hourlyRateEffectiveAt(repId: number, atUtc: string): { rateCents: number | null; effectiveFrom: string | null } {
  ...
  // "always effective" (legacy seed) — lowest precedence in BOTH passes (seq −1
  // so it never cancels real history).
  ...
  if (rep && rep.hourlyRateCents != null) {
    ...
    if (Number.isFinite(effMs)) entries.push({ rateCents: rep.hourlyRateCents, effMs, seq: Number.MAX_SAFE_INTEGER });
    else entries.push({ rateCents: rep.hourlyRateCents, effMs, seq: -1 });
  }
  // SUPERSEDE RULE: a LATER write with an earlier-or-equal effectiveFrom cancels
  // pending FUTURE-dated changes (the manager's newest statement of "the rate,
  // from when" wins). Without this, remove-then-re-add (null eff Jun 20, then
  // $25 re-added eff Jun 1) left the stale null governing weeks ≥ Jun 20 while
  // the member row showed $25 — silent underpayment. Walk newest → oldest; any
  // entry dated after the newest-yet effectiveFrom is a cancelled pending change.
  const live: typeof entries = [];
  let minEff = Number.POSITIVE_INFINITY;
  for (const e of [...entries].sort((a, b) => b.seq - a.seq)) {
    if (e.effMs > minEff) continue; // superseded future-dated change
    live.push(e);
    if (e.effMs < minEff) minEff = e.effMs;
  }
...
// Move a disputed legacy commission through the PLANNER path
// (planLegacyCommissionTransition via storage.transitionLegacyCommission) so the
// lifecycle rules, optimistic concurrency, and the audit event all hold — never
// a raw UPDATE. Illegal moves (e.g. paid is terminal) are rejected by the
// planner and simply leave the row where it is.
function transitionDisputedCommission(tenantId: number, actorId: number | null, commissionId: number, target: "disputed" | "pending" | "approved") {
  const c = storage.getCommissionById(commissionId, tenantId) as any;
  if (!c || c.status === target) return;
  storage.transitionLegacyCommission({
    id: commissionId, tenantId, actorUserId: actorId ?? 0,
    expectedRevision: c.revision, expectedStatus: c.status, status: target,
  });
}
...
export function openPayDispute(tenantId: number, actorId: number | null, input: {
  ...
  // The key is scoped to (tenant, rep) in effect: a replay from a DIFFERENT rep
  // in the same tenant never returns the original rep's row — it's a 409.
  const idemConflict = (prior: any) => {
    if (Number(prior.rep_id) !== Number(input.repId)) {
      throw new HourlyPayError("INVALID_PAY_DISPUTE", "Idempotency key already used by another dispute.", 409);
    }
    return { dispute: prior, duplicate: true };
  };
  if (idemKey) {
    const prior = rawDb.prepare(`SELECT * FROM pay_disputes WHERE tenant_id = ? AND idem_key = ?`).get(tenantId, idemKey);
    if (prior) return idemConflict(prior);
  }
  // Remember the commission's pre-dispute status so an 'upheld' resolution can
  // restore it through the lifecycle planner.
  const prevStatus = commissionId != null ? ((storage.getCommissionById(commissionId, tenantId) as any)?.status ?? null) : null;
  let dispute: any;
  try {
    const info = rawDb.prepare(
      `INSERT INTO pay_disputes (tenant_id, rep_id, week_start, line_kind, commission_id, commission_prev_status, message, status, idem_key, created_at)
       VALUES (?,?,?,?,?,?,?, 'open', ?, ?)`,
    ).run(tenantId, input.repId, input.weekStartUtc, lineKind, commissionId, prevStatus, input.message.trim().slice(0, 5000), idemKey, new Date().toISOString());
  ...
  // The disputed commission is now visibly under dispute (planner path:
  // pending|approved → disputed is legal; paid is terminal and left alone).
  if (commissionId != null) transitionDisputedCommission(tenantId, actorId, commissionId, "disputed");
...
export function resolvePayDispute(tenantId: number, actorId: number | null, id: number, input: {
  ...
  if (dispute.commission_id != null) {
    if (input.resolution === "upheld") {
      // Rep was wrong: restore the commission's PRE-DISPUTE status through the
      // lifecycle planner (a stranded 'disputed' row is non-payable and
      // non-blocking). Paid/terminal rows are left alone by the planner.
      const prev = dispute.commission_prev_status;
      if (prev === "pending" || prev === "approved") {
        transitionDisputedCommission(tenantId, actorId, dispute.commission_id, prev);
      }
    } else {
      // 'adjusted': the commission stays under dispute — the manager settles the
      // row itself through the existing commission PATCH flow after the
      // adjustment lands on a statement.
      transitionDisputedCommission(tenantId, actorId, dispute.commission_id, "disputed");
    }
  }
