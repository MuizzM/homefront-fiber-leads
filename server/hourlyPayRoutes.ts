...
export function registerHourlyPayRoutes(app: Express, deps: Deps) {
  ...
  app.post("/api/pay/punch-corrections", requireCapability("commission.adjustments.write"), (req, res) => {
    ...
    // Self-deal guard (mirrors the rate endpoint): a manager linked to a team
    // member may not pad their OWN hours — another manager must do it.
    if (user?.teamMemberId != null && Number(user.teamMemberId) === Number(repId)) {
      return res.status(403).json({ error: "You cannot correct your own punches", code: "COMMISSION_SELF_DEAL" });
    }
    try {
