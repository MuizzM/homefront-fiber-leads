...
// ── CNS lookup via dfAddressId field ─────────────────────────────────────────
async function lookupCns(env: string, cns: number, token: string): Promise<{
  found: boolean;
  data?: any;
  result?: CnsResult;
}> {
  // dfAddressId format: ENV + zero-padded 7-digit CNS
  ...
  for (let cns = startCns; cns <= endCns; cns++) {
    ...
    try {
      ...
      if (found && result) {
        ...
        if (result.isNewFiber) {
          job.newFiberHits++;
          // Auto-save NEW FIBER addresses as leads
          try {
            storage.createLead({
