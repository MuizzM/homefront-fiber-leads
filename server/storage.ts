...
export function runMigrations() {
  ...
  const stmts = [
    ...
    // ── CNS Scanner command center ──────────────────────────────────────────
    `CREATE TABLE IF NOT EXISTS cns_scan_jobs (
       id TEXT PRIMARY KEY,
       tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
       label TEXT NOT NULL,
       environment TEXT NOT NULL,
       start_cns INTEGER NOT NULL,
       end_cns INTEGER NOT NULL,
       end_inclusive INTEGER NOT NULL DEFAULT 1,
       candidate_count INTEGER NOT NULL,
       current_cns INTEGER NOT NULL,
       checked_count INTEGER NOT NULL DEFAULT 0,
       match_count INTEGER NOT NULL DEFAULT 0,
       primary_match_count INTEGER NOT NULL DEFAULT 0,
    ...
    `CREATE INDEX IF NOT EXISTS idx_cns_jobs_tenant_time ON cns_scan_jobs(tenant_id, created_at DESC)`,
    `CREATE INDEX IF NOT EXISTS idx_cns_jobs_active ON cns_scan_jobs(tenant_id, status, heartbeat_at DESC)`,
    `CREATE TABLE IF NOT EXISTS cns_scan_observations (
       id INTEGER PRIMARY KEY AUTOINCREMENT,
       tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
       job_id TEXT NOT NULL REFERENCES cns_scan_jobs(id) ON DELETE CASCADE,
       environment TEXT NOT NULL,
       control_number INTEGER NOT NULL,
       formatted_cns TEXT NOT NULL,
       scan_target_id INTEGER REFERENCES scan_targets(id) ON DELETE SET NULL,
       provider_address_id TEXT,
       classification TEXT NOT NULL,
       available INTEGER,
       technology TEXT,
       max_qualification INTEGER,
       response_hash TEXT NOT NULL,
       raw_response_json TEXT NOT NULL,
       change_detected INTEGER NOT NULL DEFAULT 0,
       confirmation_status TEXT NOT NULL DEFAULT 'primary_match',
       observed_at TEXT NOT NULL DEFAULT (datetime('now')),
       UNIQUE(job_id, environment, control_number, response_hash)
     )`,
    `CREATE INDEX IF NOT EXISTS idx_cns_obs_address ON cns_scan_observations(tenant_id,scan_target_id,observed_at DESC)`,
    `CREATE INDEX IF NOT EXISTS idx_cns_obs_job ON cns_scan_observations(tenant_id,job_id,control_number)`,
    `CREATE TABLE IF NOT EXISTS cns_job_events (
       sequence INTEGER PRIMARY KEY AUTOINCREMENT,
       tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
       job_id TEXT NOT NULL REFERENCES cns_scan_jobs(id) ON DELETE CASCADE,
       event_type TEXT NOT NULL,
    ...
    `CREATE TABLE IF NOT EXISTS cns_scanner_settings (
       ...
       job_size_limit INTEGER NOT NULL DEFAULT 100000,
       recheck_interval_hours INTEGER NOT NULL DEFAULT 24,
       retry_count INTEGER NOT NULL DEFAULT 3,
       request_timeout_ms INTEGER NOT NULL DEFAULT 8000,
       heartbeat_interval_seconds INTEGER NOT NULL DEFAULT 5,
       stale_heartbeat_seconds INTEGER NOT NULL DEFAULT 120,
       page_size INTEGER NOT NULL DEFAULT 50,
       dashboard_refresh_seconds INTEGER NOT NULL DEFAULT 10,
       updated_by INTEGER REFERENCES users(id) ON DELETE SET NULL,
       updated_at TEXT NOT NULL DEFAULT (datetime('now'))
     )`,
    `CREATE TABLE IF NOT EXISTS cns_classification_rules (
       id INTEGER PRIMARY KEY AUTOINCREMENT,
       tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
       ...
       fiber_indicator INTEGER NOT NULL DEFAULT 0,
       priority INTEGER NOT NULL DEFAULT 100,
       active INTEGER NOT NULL DEFAULT 1,
       created_at TEXT NOT NULL DEFAULT (datetime('now')),
       updated_at TEXT NOT NULL DEFAULT (datetime('now')),
       UNIQUE(tenant_id,source_field,operator,expected_value)
     )`,
  ];
