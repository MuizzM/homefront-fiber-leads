...
The September 8 follow-up audit also requires MapLibre 6.8.0 and Vitest 4.1.11. MapLibre fixes an attribution HTML sanitization bypass; current app attribution strings are fixed basemap text, but the vulnerable sanitizer was still bundled. Vitest fixes development mock-server path traversal. [MapLibre advisory](https://github.com/maplibre/maplibre-gl-js/security/advisories/GHSA-jrc7-96c5-q579), [Vitest advisory](https://github.com/vitest-dev/vitest/security/advisories/GHSA-82fw-gwwq-j7x9).

MapLibre v6 uses an explicit bundled worker, named module exports and WebGL2. The lazy loader publishes a mutable compatibility facade; map constructors retain the previous overscaling behavior so rendered-feature picking does not silently change. Vite must use `?worker&url` for the worker and bundle its imported modules. Run `npm run build` followed by `npx tsx script/map-runtime-smoke.ts` with the Playwright Chromium browser installed. The smoke uses only local synthetic GeoJSON and exercises actual production assets online and after offline reload, including worker execution, clustering, pin/territory queries, style changes, attribution sanitization and removal. Validate supported physical field devices before rollout. [Upstream installation](https://github.com/maplibre/maplibre-gl-js/blob/v6.8.0/docs/index.md#installation).
