// ── Run-scoped scan stage feed (field-facing) ─────────────────────────────────
// The Scan Inspector is requireAdmin, so the rep who STARTED an area scan could
// never see why it was or wasn't moving. /api/scan/runs/:runId/stages (+ /stream)
// expose the SAME telemetry under a capability gate, narrowed to one run the
// caller's tenant owns. These tests pin the four properties that make that safe:
// capability (not admin), the tenant wall, no secret material in the payload,
// hard bounds, and SSE listener cleanup on disconnect.
...
let repSession: string;      // tenant 1, role rep  → holds scan.submit
...
describe("run-scoped scan stage feed — authorization", () => {
  it("lets a REP (capability scan.submit, not admin) read their own tenant's run", async () => {
  ...
  it("lets a manager read the same run (capability set is a superset of rep's)", async () => {
...
describe("run-scoped scan stage feed — tenant isolation", () => {
  ...
  it("does not let a tenant-1 rep reach a tenant-2 run", async () => {
    const response = await request(`/api/scan/runs/${RUN_B}/stages`, repSession);
    expect(response.status).toBe(404);
  });

  it("ignores a client-supplied tenant hint — the wall is the session's tenant", async () => {
    const response = await request(`/api/scan/runs/${RUN_B}/stages?tenantId=2&tenant_id=2`, repSession);
    expect(response.status).toBe(404);
  });

  it("404s an unknown run id", async () => {
    const response = await request(`/api/scan/runs/run_does_not_exist/stages`, repSession);
    expect(response.status).toBe(404);
  });

  it("blocks the SSE variant across the tenant wall before any stream opens", async () => {
    const response = await request(`/api/scan/runs/${RUN_B}/stages/stream`, repSession);
    expect(response.status).toBe(404);
...
describe("run-scoped scan stage feed — bounded", () => {
  it("caps rows regardless of the requested limit", async () => {
    const response = await request(`/api/scan/runs/${RUN_BOUNDED}/stages?limit=100000`, repSession);
    expect(response.status).toBe(200);
    const body = (await response.json()) as any;
  ...
  it("clamps hostile limit values instead of trusting them", async () => {
    for (const limit of ["-1", "0", "abc", "1e9", "NaN"]) {
      const body = (await (await request(`/api/scan/runs/${RUN_BOUNDED}/stages?limit=${limit}`, repSession)).json()) as any;
  ...
  it("exposes no offset/cursor, so the table cannot be walked page by page", async () => {
    const first = (await (await request(`/api/scan/runs/${RUN_BOUNDED}/stages?limit=50`, repSession)).json()) as any;
    const walked = (await (await request(`/api/scan/runs/${RUN_BOUNDED}/stages?limit=50&offset=50&after=50&page=2`, repSession)).json()) as any;
...
describe("run-scoped scan stage feed — SSE lifecycle", () => {
  it("streams only this run's events and releases its relay listener on disconnect", async () => {
    ...
    const response = await request(`/api/scan/runs/${RUN_A}/stages/stream`, repSession, { signal: controller.signal });
    expect(response.status).toBe(200);
