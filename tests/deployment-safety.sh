...
grep -q 'attempts_left=140' scripts/deploy.sh || fail "deploy health grace is shorter than measured production startup"
grep -q 'attempts_left=140' scripts/rollback.sh || fail "rollback health grace is shorter than measured production startup"
grep -q 'start_period: 300s' docker-compose.production.yml || fail "container start period is shorter than measured production startup"
