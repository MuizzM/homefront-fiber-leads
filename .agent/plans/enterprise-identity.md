...
## Admission implementation design

- Foundation checkpoint `4b9ba7c` is pushed in draft PR #233, based on the actual default branch `rep-knocking-workflow` (the initial draft attempt incorrectly named a nonexistent master branch; corrected without source or deployment effects). Continue subsequent identity work in reviewable commits on this branch; update the PR scope when complete.
- Keep a retained, tenant-bound managed-account anchor separate from local `users.active`. New identities are pending; approval authorizes the displayed lifecycle generation only. Directory disable/delete invalidates authentication state; reactivation is pending again and does not undo local offboarding or change training/team/pay state. Generic deletion/tenant transfer of managed anchors is prohibited; protocol linking never adopts an email collision.
- Use a user authentication epoch, session assurance, and hashed browser-bound continuations. OTP, SSO and MFA finalization share a transaction-only admission path. A continuation is never an app session. Count valid concurrent sessions and insert the winning session in the same writer transaction; refuse overflow without eviction.
- Current policies constrain existing sessions too. MFA assurance must originate from Homefront. Device identifiers are random pseudonymous browser identifiers and inventory labels, not a biometric authentication factor or a replacement for MFA. Default field lifetimes remain unchanged without explicit policy.
- Bind managed OTP proofs to owner/tenant/auth epoch; revoke earlier codes, pending outbox work and continuations on lifecycle transitions. A challenge insertion failure rolls back primary-proof consumption.
- Version and bind encrypted secrets to purpose, tenant, owner and revision, using dedicated configured identity keys. No production secret is generated or installed during implementation. Old binaries must not be used after policies are activated: additive schema alone cannot preserve enforcement during rollback.
- Client admission has explicit pending/MFA/session-limit outcomes; only final admission hydrates AuthUser. Keep authoritative denial distinct from transient/offline status errors. Inspect live queue teardown/owner checks before preserving any pending work across reauthentication.

## Identity core and dependency checkpoint — 2026-09-08

- Foundation PR #233 head `4b9ba7c` passed both CI quality and the actual Linux production-image native smoke (run `34282319673`). This resolves the earlier missing Linux evidence for that exact checkpoint; staging/restore/soak/canary remain pending.
- Added an isolated, **not yet installed by production migrations or connected to HTTP login**, identity core: retained managed anchors, generation-bound approval, directory transitions, owner/epoch-bound OTP metadata, hashed browser continuations, session assurance and bounded capacity checks. Lifecycle and MFA revisions revoke sessions, outstanding primary proofs, bound codes and unfinished delivery jobs transactionally. Session overflow preserves both the existing session and pending proof.
- Added TOTP enrollment, atomic replay counters, hashed one-use recovery codes and shared per-user failure limits. Added passkey registration/authentication using SimpleWebAuthn with user verification, fixed operator RP/origin, browser-bound expiring challenges, real signature verification and commit-time credential/revision checks. New factor enrollment revokes older authority and rebinds only the proof that completed enrollment. No identity secret or live provider setting has been installed.
- Independent review corrected direct directory reactivation retaining approval, security/MFA-state deletion resets, pre-session MFA timestamp handling, stale tenant/trust on a reused browser binding, and test coverage that previously simulated only serial session admission. The contention regression now uses two actual child processes blocked behind a held writer. Migration tests include a failure after DDL begins and audit failure rolling lifecycle/revocations back.
- Fresh npm audit reported MapLibre `GHSA-jrc7-96c5-q579` and Vitest `GHSA-82fw-gwwq-j7x9`. Upgraded to MapLibre 6.8.0 and Vitest 4.1.11. New identity dependencies are pinned otplib 13.5.0, SimpleWebAuthn server 14.0.1/browser 14.0.0. Only the previously reviewed transitive UUID advisory remains (three affected package entries).
- Migrated the lazy map chunk to the v6 named exports, mutable facade and Vite-bundled worker URL. Preserved pre-v6 overscaling/picking behavior explicitly at all five constructors. Fixed `clusterExpansionZoom`: native Promise workers previously bypassed its nominal two-second timer; both APIs now share the deadline and timer cleanup.
- Real production map smoke (`script/map-runtime-smoke.ts`) passed online and again after service-worker-cached offline reload: actual worker clustering, pin/territory picking, style change, dangerous adjacent attribution attributes removed, map removal, no outbound requests and no page errors. Evidence: `/tmp/homefront-identity-runtime/map-runtime-smoke.log`. This is local Chromium evidence, not physical field-device/browser certification.
- First full run on Vitest4 passed 8,147 tests and failed one source guard: the new schema's prerequisite reads used a deferred migration transaction. Corrected to `.immediate()`; guard and passkey/deadline tests passed. Passkey fixture initially exposed library string-challenge UTF-8 encoding; passing decoded challenge bytes fixed the implementation while retaining exact expected challenge checks. Latest type check passed.
- Remaining before activation: common authority/read/renew/recovery wiring, OTP issuance/verification/outbox integration, SAML/OIDC/SCIM, device/security management routes, typed login/admin/profile UI, queue ownership quarantine, bounded HTTP contracts and final review/full gate. Do not expose the core's user-id mutation functions as unauthenticated APIs or claim an end-to-end identity feature from isolated module tests.

### Review corrections and integration seams

- Full pre-review core gate passed 8,165 tests and production build (`identity-core-map-final.log`). Subsequent product/security review added browser-bound completion receipts (lost final response retries the same still-valid session), backup codes on first passkey enrollment, recovery-rotation revocation, a distinct pending-login limit with retry time and same-browser flow replacement, pre-crypto per-user reservations, credential realm CAS, and explicit first-party WebAuthn checks. Sixty-three focused regressions pass (`identity-review-regressions.log`); final full gate plus real map smoke is running in `identity-core-reviewed-full.log` / `map-runtime-reviewed-smoke.log`.
- Map review additionally found late cluster responses targeting removed/replaced maps. All four cluster callers now provide an originating-map validity predicate; the helper discards stale results. The real map smoke has a total run deadline and cleans its HTTP server even if Chromium launch fails.
- HTTP integration must account for `server/index.ts` response sanitization: random/base64 credential strings can match its JWT-looking scrubber. Use narrowly scoped intentional-secret response projections/exemptions, not a global redaction weakening. Existing CORS permits careers origins; enterprise cookie/continuation actions need their own first-party guard using configured `APP_ORIGIN`. APP_ORIGIN is already documented in `.env.example`.
- The existing CSRF middleware skips checks when there is no application session header. New continuation endpoints therefore need browser binding plus explicit first-party request validation. Passkey/SSO browser credentials must never be installed as `x-session-id`.
- Production wiring must be one coherent change: install core schema after OTP outbox; bind both ordinary and durable OTP issuance; verify owner/epoch before consuming codes; route every primary login through continuation/final admission; update common HTTP/status/SSE/recovery authority and policy-aware coalesced renewal together. Low-level storage session creation is heavily used by fixtures but has only one current production issuer (OTP), which must switch to admission.
