...
export function StatusPinChip({ color, icon: Icon, size = 32, iconTestId = "knock-status-icon", ...rest }: StatusPinChipProps): JSX.Element {
  ...
  return (
    ...
    >
      {Icon ? <Icon data-testid={iconTestId} className={`${glyph} text-white`} /> : null}
    </span>
  );
}
