...
// render as a ranked leaderboard (top-3 tinted). Tapping jumps straight to the
// exact rooftop pin and opens the field sheet on the kept-alive Field Map.
...
export default function RankedLeads() {
  ...
  const { data, isLoading, error } = useQuery<RankedResponse>({
    queryKey: ["/api/leads/ranked", "neighborhood"],
    queryFn: () => apiRequest("GET", "/api/leads/ranked?limit=50&mode=neighborhood").then((r) => r.json()),
  ...
  const openInField = (lead: RankedLead) => openLeadOnFieldMap({
    leadId: lead.id,
    ...
    <div className="overflow-hidden rounded-2xl border border-border bg-card">
      ...
      {isLoading && !data ? (
        ...
        <div className="px-4 py-8 text-center text-[13px] italic text-muted-foreground">
          No verified neighborhood batches yet. Isolated, provisional, and already-established service hits are intentionally hidden.
        </div>
      ) : (
        <>
          <HeroLead lead={hero} onOpen={() => openInField(hero)} />
          <div className="divide-y divide-border">
            {rest.map((l, i) => (
              <button
                key={l.id}
                type="button"
                onClick={() => openInField(l)}
                aria-label={`Open ${l.address}, ${l.city} on the Field Map`}
