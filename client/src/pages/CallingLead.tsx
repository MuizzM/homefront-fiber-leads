  ...
  return (
    <div className="space-y-3" data-testid="calling-decision">
      <div className={cn("flex items-start gap-3 rounded-xl border p-3",
        ...
        {decision.eligible ? <ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-emerald-400" /> : <ShieldAlert className="mt-0.5 h-5 w-5 shrink-0 text-red-400" />}
        <div className="min-w-0">
          <div className={cn("text-sm font-semibold", decision.eligible ? "text-emerald-400" : "text-red-400")}>{formatDecision("decision" in decision ? decision.decision : decision.finalStatus)}</div>
      ...
      <details className="group rounded-xl border border-border bg-background/40">
        <summary className="flex min-h-11 cursor-pointer items-center px-3 text-xs font-semibold text-muted-foreground">Rule evidence ({decision.rules.length})</summary>
  ...
  return (
    <CallingChrome>
      <div className="flex-1 px-4 pb-28 pt-4 md:px-6 md:pb-8">
        <Link href="/calling" className="mb-4 inline-flex min-h-11 items-center gap-2 text-sm font-semibold text-muted-foreground hover:text-foreground"><ArrowLeft className="h-4 w-4" /> Back to queue</Link>
          ...
          <div className="space-y-4">
            ...
            <section className="rounded-2xl border border-border bg-card p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1"><div className="text-[10px] font-semibold uppercase tracking-[0.12em] text-primary">Cross-verified fresh fiber</div><h1 className="mt-1 text-lg font-semibold tracking-tight">{candidate.address}</h1><p className="text-sm text-muted-foreground">{candidate.city}, {candidate.state} {candidate.zip}</p></div>
              </div>
              <div className="mt-4 grid grid-cols-2 gap-px overflow-hidden rounded-xl border border-border bg-border">
                <div className="bg-background/70 p-3"><div className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">Resident match</div><div className="mt-1 flex items-center gap-1.5 truncate text-sm"><UserRound className="h-3.5 w-3.5 text-muted-foreground" />{candidate.contactName || "Not verified"}</div></div>
                <div className="bg-background/70 p-3"><div className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">Protected phone</div><div className="mt-1 truncate font-mono text-sm">{candidate.maskedPhone || "Unavailable"}</div></div>
                <div className="bg-background/70 p-3"><div className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">Line validation</div><div className="mt-1 truncate text-sm">{candidate.phoneValidationStatus || "Not validated"}{candidate.lineType ? ` · ${candidate.lineType}` : ""}</div></div>
                <div className="bg-background/70 p-3"><div className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">Pipeline stage</div><div className="mt-1 truncate text-sm">{formatStage(candidate.queueStage)}</div></div>
              </div>
            </section>
            ...
            {!activeAttempt && !completed && canAttemptManual && (
              <section className="rounded-2xl border border-border bg-card p-4">
                ...
                {!authorization ? (
                  ...
                  <div className="mt-3 rounded-xl border border-emerald-500/25 bg-emerald-500/[0.08] p-3"><div className="flex items-center justify-between gap-3 text-xs"><span className="font-semibold text-emerald-400">One-use authorization ready</span><span className="font-mono tabular-nums text-muted-foreground">{expiresIn}s</span></div><Button className="mt-3 w-full" size="lg" disabled={expiresIn <= 0 || startMutation.isPending} onClick={() => startMutation.mutate()}><PhoneCall className="h-4 w-4" />{startMutation.isPending ? "Re-checking gates…" : "Reveal number & start manual attempt"}</Button></div>
                )}
            ...
            {activeAttempt && (
              <>
                <section className="rounded-2xl border border-primary/30 bg-primary/[0.06] p-4" data-testid="active-manual-attempt">
                  <div className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.12em] text-primary"><PhoneCall className="h-3.5 w-3.5" /> Manual attempt active</div>
                  ...
                  {activeAttempt.resumed && <p className="mt-2 text-[11px] text-amber-400">Resumed open attempt after navigation or refresh. The full number is not revealed again; record the outcome to close this attempt.</p>}
            ...
            {completed && <section className="rounded-2xl border border-emerald-500/25 bg-emerald-500/[0.08] p-5 text-center"><CheckCircle2 className="mx-auto h-7 w-7 text-emerald-400" /><h2 className="mt-2 text-base font-semibold text-emerald-400">Outcome saved</h2><p className="mt-1 text-sm text-muted-foreground">{completed}</p><p className="mt-2 text-xs text-muted-foreground">No next call was started.</p><Button asChild className="mt-4"><Link href="/calling">Return to queue</Link></Button></section>}
