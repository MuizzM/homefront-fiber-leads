                      ...
                      <tr key={r.repId} className="hover:bg-secondary/40 cursor-pointer transition-colors focus:outline-none focus:bg-secondary/60 focus-visible:ring-1 focus-visible:ring-primary"
                        ...
                        <td className="px-4 py-2.5">
                          ...
                          <div className="text-2xs text-muted-foreground flex items-center gap-1">
                            {r.structure === "FLAT" ? null : r.structure === "TIERED" ? null : null}
        ...
        <DialogHeader>
          <div className="flex items-center justify-between gap-3 pr-6">
            <DialogTitle className="text-base flex items-center gap-2 min-w-0">
               <span className="truncate">{row.repName} - {weekLabel}</span>
            </DialogTitle>
            <button
              type="button"
    ...
    <div className="space-y-5" role="tabpanel" aria-label="Pay reps">
      ...
      <div className="rounded-xl border border-info/15 bg-info/[0.06] px-4 py-3 flex items-start gap-2.5 text-xs text-muted-foreground">
        ...
        <div>
          ...
          {canPay && (
            <> <a href="https://dashboard.stripe.com/balance" target="_blank" rel="noreferrer" className="text-primary hover:underline inline-flex items-center gap-0.5">Open Stripe balance </a></>
          )}
        </div>
      </div>
        ...
        <>
          ...
          {!canPay && (
            <div className="border-t border-border px-4 py-3 flex items-start gap-2 text-xs text-muted-foreground bg-secondary/20" data-testid="payout-review-only">
               This batch is read-only for your role. An admin must approve and submit the payout.
            </div>
          )}
            ...
            {canPay && (
              <Button size="sm" className="h-8 bg-primary hover:bg-primary/90 text-primary-foreground text-xs"
                ...
                data-testid="pay-reps-btn">
                 Pay {payableCount} rep{payableCount === 1 ? "" : "s"}
              </Button>
            )}
          </div>
        </>
